"""Build the August 17 Cycle 1 futility-cutoff comparison tables."""
...
FILES = {
    0.95: next(DATA_DIR.glob("*ncycle1*fut0p95*dose_summary.csv")),
    0.85: next(DATA_DIR.glob("*ncycle1*fut0p85*dose_summary.csv")),
    0.75: next(DATA_DIR.glob("*ncycle1*fut0p75*dose_summary.csv")),
}
OUT_DIR = DATA_DIR / "Table and Plots"
OUTPUT = OUT_DIR / (
    "phase12_cycle1_futility_comparison_N30_"
    "fut0p95_0p85_0p75_scenarios_1_16_20_24_27_38_tables.pdf"
)
...
CUTOFFS = (0.95, 0.85, 0.75)
METHODS = tuple(
    (f"{label}, futility = {cutoff:.2f}", allocation, stage2_allocation, cutoff)
...
def read_records() -> tuple[dict[int, dict[str, list[float]]], dict[str, dict[int, dict[str, list[float] | float]]]]:
    source_rows = {cutoff: read_rows(path) for cutoff, path in FILES.items()}
    ...
    for scenario in SCENARIOS:
        baseline = [
            row for row in source_rows[0.85]
        ...
        for label, allocation, stage2_allocation, cutoff in METHODS:
            selected = [
                row for row in source_rows[cutoff]
                if int(row["Scenario"]) == scenario
...
def draw_header(pdf: canvas.Canvas, page_number: int, page_count: int) -> None:
    ...
    pdf.drawCentredString(
        PAGE_WIDTH / 2,
        PAGE_HEIGHT - 78,
        "Cycle 1 futility cutoff comparison - N = 30, Cycle_Max = 1",
    )
