## ============================================================
## run_oc_AIDE_TITE.R
## Cluster runner for TITE AIDE-CRM backends
...
##   AIDE_CRM_helper_TITE.R
##   AIDE_CRM_helper_modified.R
source("AIDE_CRM_helper_TITE.R")

## The TITE CRM backends use rjags/coda when available.
...
## Combine TITE-CRM chunks while retaining the legacy summary fields from
## get_oc_sim_AIDE_CRM_TITE().  The TITE wrapper also supplies per-trial
## quantities so selection, stopping, and duration summaries are exact.
combine_oc_AIDE_TITE_results <- function(files) {
  x <- lapply(files, readRDS)

  ntrial_total <- sum(vapply(x, function(z) z$ntrial, numeric(1)))
  ndose <- x[[1]]$ndose

  final_mtd <- unlist(lapply(x, `[[`, "final_mtd_by_trial"), use.names = FALSE)
  earlystop <- unlist(lapply(x, `[[`, "earlystop_by_trial"), use.names = FALSE)
  total_admin <- unlist(lapply(x, `[[`, "total_admin_by_trial"), use.names = FALSE)
  ...
  n_by_dose <- weighted_mean_vec("n_by_dose")
  unique_n_by_dose <- weighted_mean_vec("unique_n_by_dose")
  nipde_by_dose <- weighted_mean_vec("nipde_by_dose")
  ntox <- weighted_mean_vec("ntox")
  ...
  out$early_stop_pct <- 100 * earlystop_count / ntrial_total
  out$na_pct <- 100 * na_count / ntrial_total
  out$selpercent <- out$selection_pct
  out$percentstop <- 100 * stop_count / ntrial_total
  out$percentna <- out$na_pct
  ...
  out$nipde_by_dose <- nipde_by_dose
  out$npatients <- n_by_dose
  out$nuniquepatients <- unique_n_by_dose
  out$nipde <- nipde_by_dose
  out$ntox <- ntox

  out$total_admin <- total_admin
  out$total_unique <- total_unique
  out$duration <- duration
  out$totaln <- mean(total_admin, na.rm = TRUE)
  out$total_unique_mean <- mean(total_unique, na.rm = TRUE)
  out$duration_mean <- mean(duration, na.rm = TRUE)

  out$raw <- NULL
  class(out) <- "oc_AIDE_TITE"
  out
}
...
## Per-dose enrollment cap for non-TITE compatibility; TITE stops only at Nmax_eff.
d_cap_aide <- 100L
dose_cap_aide <- 3L
...
## TITE helper supports the CRM backends documented in the revised model note.
...
##   ipcrm        : logistic cumulative-dose IPCRM
crm_r_model_list <- c("fixed", "random", "alpha_crm", "ipcrm")
## For quick tests, use for example:
## crm_r_model_list <- c("alpha_crm")
## crm_r_model_list <- c("ipcrm")
...
## TITE CRM JAGS models.
crm_model_file_default <- NULL
crm_fixed_model_file_default <- "fix_CRM_TITE.bug"
crm_random_model_file_default <- "random_CRM_TITE.bug"
...
crm_cumu_model_file_default <- "cumu_CRM_TITE.bug"
...
if (isTRUE(use_random_scenario_file)) {
  ...
  if (!identical(as.numeric(random_target), as.numeric(target_BOIN))) {
    warning("random_target differs from target_BOIN; run_oc_AIDE_TITE will still use target_BOIN for design decisions.")
  }
...
run_one_aide_task <- function(task) {
  ...
  with_sink(logfile, {
    ...
    if (task$model == "CRM") {
      ...
      if (task$crm_r_model %in% c("cumu_crm", "ipcrm")) {
        cat("IPCRM dose scores:", paste(task$crm_dose_scores, collapse = ", "), "\n")
        cat("IPCRM beta0 mean/prec/df:", task$crm_cumu_beta0_mean, task$crm_cumu_beta0_prec, task$crm_cumu_beta0_df, "\n")
        cat("IPCRM beta1 shape/rate:", task$crm_cumu_beta1_shape, task$crm_cumu_beta1_rate, "\n")
        cat("IPCRM beta2 rate:", task$crm_cumu_beta2_rate, "\n")
        cat("IPCRM include current:", task$crm_cumu_include_current, "\n")
      }
    }
...
outdir_root <- paste0("oc_results_cluster_AIDE_TITE_", scenario_set_name)
...
for (g in names(groups)) {
  ...
  combined <- combine_oc_AIDE_TITE_results(files)
...
cat("\nAll AIDE TITE cluster runs completed.\n")
