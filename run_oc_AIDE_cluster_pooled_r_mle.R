## ============================================================
## run_oc_AIDE_cluster.R
## Cluster runner for AIDE-BOIN / AIDE-CRM
## Updated for pooled-truncated r_mle BOIN code
...
source("AIDE_BOIN_helper_pooled_r_mle.R")
source("AIDE_CRM_helper_modified.R")
source("AIDE_modified_pooled_r_mle.R")
...
make_aide_folder <- function(task) {
  ...
  if (task$model == "BOIN") {
    method_tag <- paste0(task$decision_method, "-", task$r_estimator)
    crm_tag <- "none"
  } else {
    method_tag <- paste0("crm_", task$crm_r_model)
    crm_tag <- paste0(
      "skeleton-", paste(fmt_num(task$crm_skeleton), collapse = "_"),
      "-alphasd-", fmt_num(task$crm_alpha_sd),
      "-ar-", fmt_num(task$crm_a_r),
      "-br-", fmt_num(task$crm_b_r)
    )
  }
  ...
  paste0(
    ...
    "-method-", method_tag,
    "-crm-", crm_tag,
    "-target-", fmt_num(task$target),
    "-alphaTrue-", fmt_num(task$alpha_true),
    "-rest-", ifelse(is.null(task$r_estimator), "NA", task$r_estimator),
    "-rcarry-", fmt_num(task$r_carry),
    "-w-", task$T_assess,
    "-c-", task$C,
    "-cyc-", task$cycle_max,
    "-rate-", fmt_num(task$arrival_rate),
    "-Nmax-", task$Nmax_eff,
    "-dosecap-", task$dose_cap
  )
}
...
r_estimator_list_boin <- c("r_mle")
## r_estimator_list_boin <- c("r_fixed", "r_mle")

## CRM r versions.
crm_r_model_list <- c("random", "fixed", "level")
## crm_r_model_list <- c("fixed")
## crm_r_model_list <- c("random")

## CRM settings.
crm_skeleton_default <- c(0.15, 0.20, 0.30, 0.35, 0.45)
crm_alpha_sd_default <- 2
...
r_carry_list_boin <- c(0.1)

## For CRM or BOIN with r_estimator = "r_fixed", r_carry is used.
r_carry_list_crm <- c(0, 0.1, 0.2)
r_carry_list_fixed_boin <- c(0, 0.1, 0.2)

## Scenario set.
## Use your real scenario object if already defined by AIDE.R.
if (!exists("scenarios")) {
  scenarios <- rbind(
...
outdir_root <- "oc_results_cluster_AIDE"
...
group_key <- vapply(
  ...
  function(z) {
    ...
    paste(
      "SC", z$scenario_id,
      paste0("model", z$model),
      paste0("method", method_tag),
      paste0("alphaTrue", fmt_num(z$alpha_true)),
      paste0("rcarry", fmt_num(z$r_carry)),
      paste0("rate", fmt_num(z$arrival_rate)),
      sep = "_"
    )
  },
  character(1)
