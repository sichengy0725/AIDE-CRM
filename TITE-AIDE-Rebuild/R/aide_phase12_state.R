...
aide_phase12_initialize_state <- function(config, scenario, seed = NULL) {
  ...
  state <- list(
    ...
    admin = aide_empty_df(c("admin_id", "patient_id", "cohort_id", "decision_id", "stage", "assignment_type", "dose", "decision_next_dose", "previous_dose", "cycle", "t_arrival", "t_start", "t_dlt", "t_response", "assessment_end", "dlt_final", "eff_final", "true_p_tox", "true_p_eff")),
    ...
    logs = list(event_log = aide_empty_df(c("event_id", "time", "seq", "event_type", "patient_id", "admin_id", "action")),
                decision_log = aide_empty_df(c("decision_id", "time", "trigger_event_id", "current_dose", "stage", "action", "next_dose", "n_eval_blocked", "trigger_type", "trigger_disposition", "n_eval_required", "n_eval_observed", "stop_trial", "stop_reason")),
