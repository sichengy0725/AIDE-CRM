## ============================================================
## Extract AIDE TITE OC from cluster jobs
## Matched to run_oc_AIDE_TITE.R
##
## Newest runner naming rules:
##
## Folder:
##   oc_results_cluster_AIDE_TITE_{scenario_set_name}/
...
##
## Supports:
##   CRM : fixed / random / alpha_crm / ipcrm
## ============================================================
...
## Scenario set matched to the current 5-dose branch in run_oc_AIDE_TITE.R.
scenario_set_name <- "Set_5dose_adaptive_r_37"
results_root <- paste0("oc_results_cluster_AIDE_TITE_", scenario_set_name)
...
## Settings shown in the current 5-dose TITE run.  Update these values if the
## corresponding run_oc_AIDE_TITE.R settings change.
scenarios <- 1:6
...
arrival_rate_list <- c(1 / 50, 1 / 25, 1 / 14)
...
## TITE CRM backends, matching run_oc_AIDE_TITE.R.
crm_r_model_list <- c("fixed", "random", "alpha_crm", "ipcrm")

## r values by CRM backend, matching the names produced by run_oc_AIDE_TITE.R.
## fixed CRM uses r_carry; alpha_crm/ipcrm do not use fixed r, so the
...
## CFO / PRIDE settings are retained unchanged from extract_result_AIDE_5dose.R.
...
## run_oc_AIDE_TITE.R contains CRM backends only.
model_list <- c("CRM")

out.dir <- paste0("OC_summary_from_parallel_AIDE_TITE_jobs_", scenario_set_name)
...
extract_est_pj <- function(res.list, model, ndose) {
  if (model == "CRM") {
    return(extract_mean_vector(
      ...
      ndose = ndose,
      ## Current TITE combined files do not retain posterior p estimates.
      ## Preserve the standard output column with NA rather than failing.
      required = FALSE
    ))
  }
...
summarize_aide_files <- function(files.use,
  ...
  if (model == "CRM" && crm_r_model %in% c("cumu_crm", "ipcrm")) {
...
out.tag <- paste0(
  paste0("All_AIDE_TITE_OC_", scenario_set_name),
