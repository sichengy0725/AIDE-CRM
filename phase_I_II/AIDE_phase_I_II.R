...
##      utility allocation among the refreshed Stage II admissible doses.
##   2. "one_stage": utility ranks every dose at or below the current MTD;
##      escalation toward the provisional OBD cannot skip an untried dose.
##
...
simulate_AIDE_phase_I_II <- function(
  ...
  choose_one_stage_dose <- function(allocation_state) {
    ...
    if (target_dose <= highest_tried_dose) return(target_dose)
    min(allocation_state$admissible[
      allocation_state$admissible > highest_tried_dose &
        allocation_state$admissible <= target_dose
    ])
  }

  choose_stage2_doses <- function(allocation_state, n_to_enroll) {
    ranked_doses <- allocation_state$admissible[
      order(-allocation_state$utility[allocation_state$admissible],
            allocation_state$admissible)
    ]
    if (stage2_allocation == "highest_utility") {
      return(list(
        dose = rep.int(ranked_doses[1L], n_to_enroll),
        probabilities = paste0("D", ranked_doses[1L], "=1")
      ))
    }
  ...
  if (allocation == "two_stage") {
    ...
    ## computes utility on every admissible dose, and allocates to the
    ## provisional OBD. Upward movement cannot skip an untried dose.
    while (nrow(admin) < Nmax && !earlystop) {
      futility_before <- futility_state()
