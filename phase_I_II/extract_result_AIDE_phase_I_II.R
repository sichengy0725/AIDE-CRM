...
results_root <- paste0("oc_results_cluster_phase_I_II_", scenario_set_name)
out_dir <- "OC_summary_phase_I_II"
...
make_metadata <- function(result, task_id) {
  ...
  data.frame(
    ...
    IPDE_Dose_Assignment = "any_admissible_dose",
...
for (task_row in seq_len(nrow(expected_tasks))) {
  ...
  cat("IPDE dose assignment: any admissible dose\n")
...
make_phase12_output_tag <- function() {
  ...
  paste0(
    ...
    "_rate", paste(
      fmt_short(1 / unique(arrival_grid$arrival_rate)),
      collapse = "_"
    ), "d",
    "_ep", paste(efficacy_priors, collapse = "_"),
    "_cp", paste(carryover_priors, collapse = "_"),
    "_eth", fmt_short(efficacy_threshold),
    "_fut", fmt_short(futility_cutoff),
