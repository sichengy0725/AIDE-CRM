...
crm_fit_discount <- function(dat,
  ...
  if (length(cutoff.eli) != 1L ||
      !is.finite(cutoff.eli) ||
      cutoff.eli <= 0 ||
      cutoff.eli >= 1) {
    stop("cutoff.eli must be a scalar in (0, 1).")
  }
  if (r_model == "fixed" &&
      (length(r_carry) != 1L ||
       !is.finite(r_carry) ||
  ...
  ## Stop only if posterior Pr(p_1 > target | data) > cutoff.eli.
  prob_p1_over_target <- mean(post[, "p[1]"] > target)
  
  earlystop <- is.finite(prob_p1_over_target) &&
    prob_p1_over_target > cutoff.eli
  
  eliminated <- if (earlystop) {
    rep(1L, ndose)
  } else {
    rep(0L, ndose)
  }
  ...
  list(
    ...
    skeleton = skeleton,
    prob_p1_over_target = as.numeric(prob_p1_over_target),
...
crm_move <- function(current_dose,
  ...
  ## Stop the trial if dose 1 is overly toxic according to CRM.
  ## The earlystop decision is computed inside crm_fit_discount().
  if (isTRUE(as.logical(fit$earlystop))) {
    return(list(
      next_dose = 99L,
...
select.mtd.crm <- function(target,
  ...
  ## CRM elimination is determined inside crm_fit_discount().
  ## It is based only on posterior Pr(p_1 > target | data) > cutoff.eli.
  ## Do not reuse BOIN beta-binomial elimination here.
  if (isTRUE(as.logical(fit$earlystop))) {
    elimi <- fit$eliminated
