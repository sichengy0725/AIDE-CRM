## ============================================================
## Selected S20_5d scenarios: toxicity curves and OC plots
## ============================================================
...
input_file <- file.path(
  "Results",
  "All_AIDE_OC_S20_5d_modelsCFO_CRM_crmfixed_random_level_alpha_crm_cumu_crm_cfopride_target0p3_w28_c3_cyc1_2_3_rate0p07_Nmax30_45_dosecap3_cont1_tried1_jobs1to2000_dose_summary.csv"
)

out_dir <- file.path("Results", "S20_5d_selected_mtd_scenarios")
...
selected_input <- data.frame(
  ...
  Original_Scenario = c(6L, 10L, 2L, 4L, 20L, 9L, 5L),
  Given_MTD = c(2L, 3L, 4L, 5L, 3L, 4L, 2L)
)
...
tox_pdf <- file.path(out_dir, "S20_5d_selected_scenarios_toxicity_curves.pdf")
pdf(tox_pdf, width = 9.5, height = 6.5, onefile = FALSE, useDingbats = FALSE)
par(mar = c(4.3, 4.5, 3.0, 1.0), xpd = NA)
plot(
  c(1, 5), c(0, max(0.9, max(tox_rows$True_DLT_rate, na.rm = TRUE))),
  type = "n",
  xlab = "Dose Level",
  ylab = "True DLT probability",
  main = "Selected S20_5d Toxicity Curves",
  axes = FALSE
)
axis(1, at = 1:5)
axis(2)
...
plot_one_setting <- function(alpha_value, ncycle, nmax_eff) {
  ...
  out_file <- file.path(
    out_dir,
    paste0(
      "S20_5d_selected_CRM_OC_alpha_ipcrm_random_cfo_pride_",
  ...
  text(
    0.5, 0.48,
    "Oracle CRM uses fixed CRM with r = 0, carryover alpha = 0, ncycle = 1, and the same Nmax.",
    cex = 0.82
  )
  text(
    0.5, 0.18,
  ...
  mtext(
    paste0(
      "Selected S20_5d Operating Characteristics (alpha = ",
